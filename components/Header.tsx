 ...
 * unmounting.
 *
 * Off the home page — the legal documents — everything but the wordmark drops
 * away. The nav, its hamburger and the overlay only address sections of the
 * home page, and "Let's Talk" belongs to the pitch, not to a policy someone is
 * reading. What is left is the mark, which goes home.
 */
export function Header() {
  ...
  // Lock body scroll while the overlay menu is open. Written only while the
  // menu actually holds the lock, never on the way past: the loading curtain
  // takes the same lock on every route change, and an unconditional `= ""`
  // here would release the curtain's the moment a navigation re-ran this.
  // Leaving the home page unmounts the overlay, so that releases it too.
  useEffect(() => {
  ...
  return (
    <>
      <header className="fixed inset-x-0 top-0 z-[80]">
        {/* Reading scrim: blur plus a light wash of whichever surface the
            header is over, so the wordmark and "Let's Talk" stay legible over
            page content. Masked out at its foot to avoid a hard edge.
            `pointer-events-none` also keeps it invisible to `isDarkSurfaceAt`,
            which probes this strip to pick the header's own colour. */}
        <div
          aria-hidden
          className={cn(
        ...
        <div className="relative mx-auto flex max-w-[90rem] items-center justify-between gap-4 px-5 py-4 sm:px-8">
          {onHome ? (
            ...
            <Link
              ...
              aria-label={`${SITE.shortName}, back to the home page`}
            >
              <Logo title={null} className="h-8 w-auto sm:h-9" />
            </Link>
          )}
          ...
          {onHome && (
            <>
              ...
              <div className="flex items-center gap-3">
                ...
                {/*  Contact CTA.                                             */}
                {/*                                                           */}
                {/*  Set as type on an accent rule rather than as a filled     */}
                {/*  pill. A yellow rounded-full button in the corner of every */}
                {/*  screen is the single most templated element a portfolio   */}
                {/*  can carry, and it was competing with the hero's own       */}
                {/*  primary button for the same colour and the same job. As a */}
                {/*  marked-up word it still reads as the one thing to press,  */}
                {/*  without pretending to be a product's sign-up.             */}
                {/*                                                           */}
                {/*  The rule is the always-on part; hover thickens it and     */}
                {/*  moves the arrow, so the affordance never depends on hover */}
                {/*  alone.                                                    */}
                {/* -------------------------------------------------------- */}
                <a
                  href="#contact"
      ...
      {/* Full-screen mobile menu. Mounted for the whole of the home page;
          `autoAlpha` hides it. */}
      {onHome && (
        <div
          ref={overlayRef}
